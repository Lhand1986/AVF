...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kAVFWK1ProjectHandLevi1508UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kAVFWK1ProjectHandLevi1508UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a AVFWK1ProjectHandLevi1508 app using the node.js CLI
