...
//Rare exceptions to indicate a bug in the _avfwk3projecthandlevi1508 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_avfwk3projecthandlevi1508 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_avfwk3projecthandlevi1508]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.avfwk3projecthandlevi1508.module"
