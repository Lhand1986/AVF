...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kAVFWk3ProjectHandLevi1508UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kAVFWk3ProjectHandLevi1508UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a AVFWk3ProjectHandLevi1508 app using the node.js CLI
