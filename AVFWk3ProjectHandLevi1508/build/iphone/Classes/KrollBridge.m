...
NSString * AVFWk3ProjectHandLevi1508$ModuleRequireFormat = @"(function(exports){"
...
@implementation AVFWk3ProjectHandLevi1508Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_avfwk3projecthandlevi1508);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create AVFWk3ProjectHandLevi1508 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "AVFWk3ProjectHandLevi1508" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_avfwk3projecthandlevi1508 = [[AVFWk3ProjectHandLevi1508Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_avfwk3projecthandlevi1508];
	
	NSString *_avfwk3projecthandlevi1508NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _avfwk3projecthandlevi1508NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_avfwk3projecthandlevi1508 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:AVFWk3ProjectHandLevi1508$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.avfwk3projecthandlevi1508.kroll" 
