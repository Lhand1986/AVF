...
//Rare exceptions to indicate a bug in the _handleviavfgtt code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_handleviavfgtt = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_handleviavfgtt]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.handleviavfgtt.module"
