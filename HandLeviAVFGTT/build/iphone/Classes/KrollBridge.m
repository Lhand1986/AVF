...
NSString * HandLeviAVFGTT$ModuleRequireFormat = @"(function(exports){"
...
@implementation HandLeviAVFGTTObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_handleviavfgtt);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create HandLeviAVFGTT global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "HandLeviAVFGTT" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_handleviavfgtt = [[HandLeviAVFGTTObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_handleviavfgtt];
	
	NSString *_handleviavfgttNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _handleviavfgttNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_handleviavfgtt valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:HandLeviAVFGTT$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.handleviavfgtt.kroll" 
